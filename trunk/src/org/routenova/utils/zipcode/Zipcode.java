 ...
 * @version     $Id: Zipcode.java 11 2007-01-21 19:40:52Z mikhail.branicki $
...
public class Zipcode {
    ...
    public boolean isZipcode(String zipcode) {
        ...
        // look for contacts
        try {
            stmt = db.prepareStatement("SELECT * FROM "
                    + dbScheme
    ...
    public Double findDistance(String fromZip, String toZip) {
        ...
        // look for contacts
        try {
            stmt = db.prepareStatement("SELECT lat,lon FROM "
                    + dbScheme
                    + ".zipcode WHERE zipcode = ?");
        ...
        distance = Math.acos( Math.sin(lat_src) * Math.sin(lat_dst) +
                Math.cos(lat_src) * Math.cos(lat_dst) *
                Math.cos(lon_dst-lon_src)) * 6372.797;
    ...
    public boolean findRadius(String zipcode, Integer distance) {
        ...
        try {
            ...
            //Execute zip search stored procedure
            stmt = db.prepareCall("{ ? = call " + dbScheme 
                    + " .get_zipradius(?,?) }", ResultSet.TYPE_SCROLL_SENSITIVE,
